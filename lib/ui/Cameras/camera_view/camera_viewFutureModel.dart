...
class CameraViewFutureModel extends FutureViewModel<List<dynamic>> {
  ...
  void makeAdress(){
    ...
    String rtsp = Get.arguments.rtsp;
    for(int j = 1; j<=Get.arguments.nbrCamera; j++)
    {
      print("inside boucle");
  ...
  Future<List<void>> initPlayer() async {
    ...
    print("made adresses");
    print(Get.arguments.nbrCamera);
    makeAdress();
    for(int i=0; i<Get.arguments.nbrCamera; i++) {
      print("controller 1");
      _videoPlayerControllers.add(VlcPlayerController.network(_rtspList[i],
          autoPlay: true,
